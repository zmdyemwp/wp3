...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    private OnGlobalLayoutListener orientationChangeListener = new OnGlobalLayoutListener() {
        ...
        public void onGlobalLayout() {
            ////Note:Android 4.2 deprecate removeGlobalOnLayoutListener(),add removeOnGlobalLayoutListener().
            // we should use the later
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (deltaX < 0) {
                    ...
                    final int availableToScroll = getChildAt(getChildCount() - 1).getRight()
                            - scrollX - getWidth();
                    //LisaYi add this condition to avoid show empty view on right begin
                    if (scrollX < (getChildCount() - 1) * getWidth()) {
                    //LisaYi add this condition to avoid show empty view on right end
            ...
            if (mLazyInit.contains(LazyInit.RIGHT)) {
                ...
                if (mCurrentBufferIndex + 1 < mLoadedViews.size())
                    mViewInitializeListener.onViewLazyInitialize(
                            mLoadedViews.get(mCurrentBufferIndex + 1), mCurrentAdapterIndex + 1);
            }
        } else {
    ...
    private void resetFocus() {
        ...
        for (int i = Math.max(0, mCurrentAdapterIndex - mSideBuffer); i < Math.min(
            ...
            if (i == mCurrentAdapterIndex) {
                ...
                if (mViewInitializeListener != null)
                    mViewInitializeListener.onViewLazyInitialize(mLoadedViews.getLast(),
    ...
    private void postViewSwitched(int direction) {
        ...
        if (mIndicator != null) {
            // LisaYi modify for resolve IndexOutOfBoundsException
            mIndicator.onSwitched(mLoadedViews.get(mCurrentScreen/*mCurrentBufferIndex*/),
                    mCurrentAdapterIndex);
        }
        if (mViewSwitchListener != null) {
            // LisaYi modify for resolve IndexOutOfBoundsException
            mViewSwitchListener
                    .onSwitched(mLoadedViews.get(mCurrentScreen/*mCurrentBufferIndex*/),
                            mCurrentAdapterIndex);
        }
        logBuffer();
    }
    ...
    private void logBuffer() {
        ...
        LogApp.Logd(TAG,
                "Size of mLoadedViews: " + mLoadedViews.size() + ", Size of mRecycledViews: "
                        + mRecycledViews.size() + ", X: " + mScroller.getCurrX() + ", Y: "
                        + mScroller.getCurrY());
        LogApp.Logd(TAG, "IndexInAdapter: " + mCurrentAdapterIndex + ", IndexInBuffer: "
                + mCurrentBufferIndex);
    }
}
