...
public class IconListPreference extends ListPreference {
    ...
    private int mInitIconIds[];
    private int mInitLargeIconIds[];
    private int mInitImageIds[];
    ...
    public IconListPreference(Context context, AttributeSet attrs) {
		...
		//2012/3/24 Jinsheng add for IRMI.B-1781 begin		
        mInitIconIds = getIds(res, a.getResourceId(
                R.styleable.IconListPreference_icons, 0));
        mInitLargeIconIds = getIds(res, a.getResourceId(
                R.styleable.IconListPreference_largeIcons, 0));
        mInitImageIds = getIds(res, a.getResourceId(
		//2012/3/24 Jinsheng add for IRMI.B-1781 end
                R.styleable.IconListPreference_images, 0));
        a.recycle();
    }
    ...
    public void filterUnsupportedDynamic(List<String> supported) {
        super.filterUnsupportedDynamic(supported);
        CharSequence entryValues[] = getEntryValues();
        IntArray iconIds = new IntArray();
        IntArray largeIconIds = new IntArray();
        IntArray imageIds = new IntArray();
        CharSequence initEntryValues[] = getInitEntryValues();
        int index = 0;

        for (int i = 0, len = entryValues.length; i < len; i++) {
            if (supported.indexOf(entryValues[i].toString()) >= 0) {
                
                for(int j = i,initLen = initEntryValues.length; j < initLen; j++){
                    if(initEntryValues[j].equals(entryValues[i])){
                        index = j;
                    }
                }
                
                if (mIconIds != null) iconIds.add(mInitIconIds[index]);
                if (mLargeIconIds != null) largeIconIds.add(mInitLargeIconIds[index]);
                if (mImageIds != null) imageIds.add(mInitImageIds[index]);
            }
        }
        if (mIconIds != null) mIconIds = iconIds.toArray(new int[iconIds.size()]);
        if (mLargeIconIds != null) {
