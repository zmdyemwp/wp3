...
public class FocusManager {
    ...
    public void onShutterDown() {
        ...
        if (needAutoFocusCall()) {
            Log.i(TAG,"needAutoFocusCall,check if touch focus done");
            // Do not focus if touch focus has been triggered.
            if (mState != STATE_SUCCESS && mState != STATE_FAIL) {
                Log.i(TAG,"needAutoFocus call autoFocus:");
                autoFocus();
            }
            else{
                Log.i(TAG,"touch focus has been triggered,don't need AF");
            }
        }
        else{
            Log.i(TAG,"Don't needAutoFocusCall");
        }
    }
    ...
    public void onAutoFocus(boolean focused) {
        Log.i(TAG,"onAutoFocus focused:"+focused);
    ...
    public boolean onTouch(MotionEvent e) {
        Log.i(TAG,"onTouch mState:"+mState);
    ...
    public List<Area> getFocusAreas() {
        Area area = null;
        if(mFocusArea != null){
            area = mFocusArea.get(0);
            Log.i(TAG,"Size of mFocusArea:"+mFocusArea.size());
        }
        
        if(isFocusAreaValid(area))
            return mFocusArea;
        else 
            return null;
    }
    ...
    //2012/6/7 Jinsheng add for IPD.B-3595 begin
    public boolean isFocusAreaValid(Area area){
        boolean bRetVal = false;
        if(area==null)return bRetVal;
        Rect rc= area.rect;
        if(rc==null)return bRetVal;
        int left = rc.left; 
        int top = rc.top; 
        int right = rc.right; 
        ...
        Log.i(TAG,"Check if the focus area is valid,left:"+left+" top:"+top+" right:"+right+" bottom:"+bottom+" weight:"+weight);

        //This check is from BSP
        // left should >= -1000 
        if (!(left >= -1000)) 
            return bRetVal; 
        // top should >= -1000 
    ...
    //2012/6/7 Jinsheng add for IPD.B-3595 end
