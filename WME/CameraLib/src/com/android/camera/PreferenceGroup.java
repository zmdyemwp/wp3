...
public class PreferenceGroup extends CameraPreference {
    ...
    //2012/3/23 Jinsheng add for IRMI.B-1781 begin,color effect need dynamic update,
    //if white balance is set some color effect isn't support 
    public void updatePreference(String key,List<String> supported){
        ListPreference pref = findPreference(key);
        if(null == pref)return;
        
        pref.filterUnsupportedDynamic(supported);
        pref.reloadValue();
    }
