...
public class CameraHolder {
            ...
            switch(msg.what) {
                case RELEASE_CAMERA:
                    synchronized (CameraHolder.this) {
                        ...
                        // here:
                        Log.i(TAG,"handleMessage RELEASE_CAMERA");
                        if (CameraHolder.this.mUsers == 0) releaseCamera();
                        else Log.i(TAG,"CameraHolder.this.mUsers != 0,don't release");
                    }
                    break;
            }
        }
    ...
    public synchronized android.hardware.Camera open(int cameraId)
        ...
        Log.i(TAG,"open camera mUsers:"+mUsers+"  cameraId:"+cameraId+"  mCameraId:"+mCameraId);
        Assert(mUsers == 0);
        if (mCameraDevice != null && mCameraId != cameraId) {
            Log.i(TAG,"Release first if mCameraDevice isn't null");
        ...
        if (mCameraDevice == null) {
            ...
            try {
                Log.i(TAG,"Camera device haven't release,just reconnect it");
        ...
        ++mUsers;
        Log.i(TAG,"removeMessages RELEASE_CAMERA");
    ...
    public synchronized void release() {
        Log.i(TAG,"into release mUsers:"+mUsers);
        Assert(mUsers == 1);
        --mUsers;
    ...
    private synchronized void releaseCamera() {
        Log.i(TAG,"releaseCamera mUsers:"+mUsers);
        Assert(mUsers == 0);
        Assert(mCameraDevice != null);
        long now = System.currentTimeMillis();
        if (now < mKeepBeforeTime) {
            Log.i(TAG,"releaseCamera,send message delay to release camera");
