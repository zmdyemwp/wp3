...
public abstract class IndicatorControl extends RelativeLayout implements
    ...
    protected void addControls(String[] keys, String[] otherSettingKeys) {
        if (keys != null) {
            for (int i = 0; i < keys.length; i++) {
                ...
                if (pref != null) {
                    if(keys[i].equals(CameraSettings.KEY_COLOR_EFFECT)){
                        addIndicator(getContext(), null,pref);
                    }
                    else if(keys[i].equals(CameraSettings.KEY_VIDEO_EFFECT)){
                        IconListPreference pref2 =
                            (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_COLOR_EFFECT);
                        addIndicator(getContext(), pref,pref2);
                    }
                    else{
	...
	public IndicatorButton addIndicator(Context context, IconListPreference pref,IconListPreference secondPref) {
        IndicatorButton b = new IndicatorButton(context, pref,secondPref);
        b.setSettingChangedListener(this);
        if(pref != null){
            b.setContentDescription(pref.getTitle());
        }else if(secondPref != null){
            b.setContentDescription(secondPref.getTitle());
        }
